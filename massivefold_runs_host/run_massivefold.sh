...
\n\
  Facultative options:\n\
    ...
    -N| --cluster_size: number of gpu nodes in your cluster." 
  exit 1
fi
...
if [[ $waiting_for_alignment = true  ]]; then
  echo "Waiting for alignment"
  waiting_for_agg=true
  AGG_ID=$(sbatch --parsable --dependency=afterok:$ALIGNMENT_ID ${sequence_name}_${run_name}_jobarray.slurm)
#  ARRAY_ID=$(sbatch --parsable --dependency=afterok:$ALIGNMENT_ID ${sequence_name}_${run_name}_jobarray.slurm)
else
  echo "NOT waiting for alignment"
  AGG_ID=$(sbatch --parsable ${sequence_name}_${run_name}_jobarray.slurm)
...
until [ -f ${logs_dir}/${sequence_name}/${run_name}/${sequence_name}_${run_name}_jobarray_as_joblist.txt ] || [ "$elapsed" -ge "$timeout" ]; do
    echo "Waiting for file ${logs_dir}/${sequence_name}/${run_name}/${sequence_name}_${run_name}_jobarray_as_joblist.txt for ${elapsed} seconds"
    sleep 5
    elapsed=$((elapsed + 5))
done
elapsed=0
